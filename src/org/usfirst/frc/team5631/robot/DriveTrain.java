...
public class DriveTrain {
	...
	public void raiseElevator(double level) {
		...
		//Checks Whether the elevators needs to go up or down and applies the correct speed
	...
	public void drive(double distance1, double distance2) {
		...
		//checks whether the The distance is negative if so the speed is inverted
		if (distance1 < 0)
			speed1 *= -1;
		if (distance2 < 0)
			speed2 *= -1;
		...
		//If the current distance is less then the distance needed then it applies the speed to the motors
		...
		//once it reaches its destination the robot stops and resets the encoders distance
	...
	public void setWheelSystemSpeeds(double leftSideSpeed, double rightSideSpeed) {
		...
		//the PID then tries to regulate the read speed to get it to the needed speed
	...
	public void calibrateElevator() {
		...
		runSystems();
		//Checks if the timer is greater than 0, if so then checks if the read encoder speed
		//is 0 then the elevator has stopped moving meaning it had reached the bottom
		//therefore the distance on the encoders can be reset in order to have the distance start at the bottom
